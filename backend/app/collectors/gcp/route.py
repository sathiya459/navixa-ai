...
async def collect_routes(client: RoutesClient, project_id: str, semaphore) -> CollectionResult:
    start = time.monotonic()
    async with semaphore:
        try:
            routes = await with_gcp_retry(
                lambda: asyncio.to_thread(lambda: list(client.list(project=project_id)))
            )
            items = [proto_to_dict(route) for route in routes]
