...
- "delegated": uses the environment's own root-credential IAM Identity
  ...
  on `EnvironmentConnection.delegated_token_cache` - one popup login per
  (environment, provider), reused across every tenant/account in that
  environment. Resource collection uses `sso.GetRoleCredentials` against
  that session's access token - never the backend host's own `aws sso
  login` profile. Raises DelegatedAuthRequiredError if there's no valid
  cached session.
...
async def assume_role_for_scope(
    ...
    """In delegated mode, resolves a concrete ScopedCredentials via the
    environment's own SSO session (get_delegated_role_credentials);
    otherwise assumes `settings.aws_audit_role_name` in the target account
    (`external_scope_id`) via the platform's own calling identity
    (`connection` is unused in this branch)."""
    if settings.cloud_auth_mode == "delegated":
        return await get_delegated_role_credentials(connection, external_scope_id, region)
...
def get_org_session(region: str) -> aioboto3.Session:
    ...
    it lists accounts via the environment's SSO session instead
    (list_sso_accounts)."""
    return aioboto3.Session(region_name=region)


# --- Per-environment IAM Identity Center (AWS SSO) delegated auth --------
#
# `EnvironmentConnection.delegated_token_cache` (encrypted) holds a JSON
# blob shaped:
...
def get_sso_session_dict(connection: EnvironmentConnection) -> dict | None:
...
async def _refresh_sso_access_token(connection: EnvironmentConnection, session: dict) -> dict:
...
async def get_valid_sso_session(connection: EnvironmentConnection | None) -> dict:
    ...
    if connection is None:
        raise DelegatedAuthRequiredError("unknown", "aws")

    session = get_sso_session_dict(connection)
    if session is None:
        raise DelegatedAuthRequiredError(connection.environment, "aws")

    if int(time.time()) < session["access_token_expires_at"] - 60:
        return session

    try:
        return await _refresh_sso_access_token(connection, session)
    except Exception as exc:
        raise DelegatedAuthRequiredError(connection.environment, "aws") from exc


async def get_delegated_role_credentials(
    connection: EnvironmentConnection | None, account_id: str, region: str
) -> ScopedCredentials:
    """Mints short-lived credentials for one account via the environment's
    own SSO session - `settings.aws_audit_role_name` is reused as the IAM
    ...
    session = await get_valid_sso_session(connection)
    aio_session = aioboto3.Session(region_name=session["region"])
...
async def list_sso_accounts(connection: EnvironmentConnection | None) -> list[tuple[str, str]]:
    """Accounts the environment's SSO session can see, via Identity
    Center's own ListAccounts - not AWS Organizations, so no
    organizations:* permissions are required at all."""
    session = await get_valid_sso_session(connection)
    aio_session = aioboto3.Session(region_name=session["region"])
    accounts = []
    async with aio_session.client("sso", region_name=session["region"]) as sso:
