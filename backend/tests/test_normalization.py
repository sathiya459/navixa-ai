...
def test_normalize_skips_failed_collectors():
    results = [
        CollectionResult(resource_type="vpc", status="success", items=[{"VpcId": "vpc-1"}]),
    ...
    assert normalized[0]["resource_type"] == "vpc"
...
def test_normalize_handles_partial_status():
    results = [
        CollectionResult(resource_type="vpc", status="partial", items=[{"VpcId": "vpc-2"}]),
    ]
    normalized = normalize_aws_results(results)
    assert len(normalized) == 1
